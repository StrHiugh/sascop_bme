...
def fn_ejecutar_busqueda_global(payload):
    ...
    if not check_entregados and not check_pendientes:
        filtro_entregado, filtro_no_entregado = 1, 1
    else:
    ...
    params = {
        'fecha_ini': fecha_ini_input or '1900-01-01', 
        'fecha_fin': fecha_fin_input or '2100-12-31',
    ...
    WHERE
        ...
        AND (CASE
            WHEN %(id_sitio)s IS NULL THEN (%(frente_id)s IS NULL OR T._fid_frente = %(frente_id)s)
            WHEN %(buscar_por_frente)s = '1' THEN (T._fid_frente = %(frente_id)s AND ((%(frente_id)s = 1 AND T._fid_patio = %(id_sitio)s::int) OR (%(frente_id)s = 2 AND T._fid_embarcacion = %(id_sitio)s::int) OR (%(frente_id)s = 4 AND T._fid_plataforma = %(id_sitio)s::int)))
            ELSE (T._fid_patio = %(id_sitio)s::int OR T._fid_embarcacion = %(id_sitio)s::int OR T._fid_plataforma = %(id_sitio)s::int)
        END)
